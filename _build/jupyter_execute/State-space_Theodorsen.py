...
# ## The Theodorsen dynamical model of unsteady lift
...
# ### Impulse in $\ddot{h}$
# 
# The first example shall be the response to an $\ddot{h}$ impulse, equivalent to a step change in angle of attack (as discussed before, a change of $\dot{h}$ is equivalent to a change in effective angle of attack $\alpha_e$, therefore an impulse in $\ddot{h}$, corresponding to a step in $\dot{h}$, is equivalent to a step in $\alpha_e$). This is equivalent to the Wagner function, defined as the generalised step response to $\alpha_e$ of the lift {cite}`brunton2013empirical` (the only difference being the scaling factor of $C_2 = 2\pi$). The output is compared to an approximation of the Wagner function obtained by Venkatesan & Friedman {cite}`brunton2013empirical`:
